...
def ensure_agent_exists(
    ...
    *,
    ...
    if list_result.returncode == 0:
        if agent_id.lower() in existing_agents or normalized_id in existing_agents:
            ...
            if (
                ...
                logger.info("Agent %s already exists with correct workspace — refreshing models.json", agent_id)
                skip_create = True  # skip agents add, but still rewrite models.json below
            else:
    ...
    if base_url:
        ...
        # Always rewrite models.json (even if agent already existed) so the
        # custom provider and defaultProvider/defaultModel are guaranteed fresh.
        data: dict[str, Any] = {}
        ...
        configure_bench_models(agent_id, model_id, base_url, api_key)
...
def _patch_openclaw_json_model(agent_id: str, model_spec: str) -> None:
    """Update the agent's 'model' field in ~/.openclaw/openclaw.json.

    When using a custom provider, model_spec should be 'custom/<model_id>'
    so OpenClaw's model-selection can resolve the provider from models.json.
    """
    openclaw_cfg = Path.home() / ".openclaw" / "openclaw.json"
    if not openclaw_cfg.exists():
        return
    try:
        raw = openclaw_cfg.read_text("utf-8")
        cfg = json.loads(raw)
    except Exception as exc:
    ...
    for entry in agents_list:
        ...
        if entry_id == agent_id.lower() or entry_id == normalized_id:
            if entry.get("model") != model_spec:
    ...
    if patched:
        try:
            openclaw_cfg.write_text(json.dumps(cfg, indent=2, ensure_ascii=False), "utf-8")
            logger.info(
                "Patched openclaw.json: agent %s model -> %s", agent_id, model_spec
            )
        except Exception as exc:
            logger.warning("Failed to write openclaw.json: %s", exc)


def configure_bench_models(
    agent_id: str,
    model_id: str,
    base_url: str,
    api_key: str | None = None,
) -> None:
    """Write custom provider config to a bench agent's models.json.

    Called both during agent creation and again after a short delay to
    overwrite any async re-initialisation that openclaw may perform after
    ``agents add`` returns.
    """
    bench_agent_dir = _get_agent_store_dir(agent_id) / "agent"
    bench_agent_dir.mkdir(parents=True, exist_ok=True)
    ...
    logger.info("Wrote custom provider config to %s (model=%s)", bench_models, model_id)

    # Also patch openclaw.json so model-selection reads "custom/<model>" and
    # resolves to our registered provider instead of falling back to openai/.
    _patch_openclaw_json_model(agent_id, f"custom/{model_id}")
