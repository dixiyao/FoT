...
def _message_to_openai(
    ...
    if tool_uses:
        ...
        for tu in tool_uses:
            ...
            tool_call: dict[str, Any] = {"id": tu.id, "type": "function", "function": fn}
            if tu.thought_signature:
                # Gemini 3 OpenAI compatibility requires signatures here, not
                # inside `function`. Missing this field causes HTTP 400 on the
                # next turn after a tool call.
                tool_call["extra_content"] = {
                    "google": {"thought_signature": tu.thought_signature}
                }
...
def _extract_thought_signature_from_tool_call(tc: Any) -> str | None:
    """Find Gemini thought signatures across OpenAI SDK parsing variants."""
    candidates = [
        _lookup_path(tc, "extra_content", "google", "thought_signature"),
        _lookup_path(tc, "model_extra", "extra_content", "google", "thought_signature"),
        _lookup_path(tc, "function", "extra_content", "google", "thought_signature"),
        _lookup_path(tc, "function", "model_extra", "extra_content", "google", "thought_signature"),
        _lookup_path(tc, "thought_signature"),
        _lookup_path(tc, "model_extra", "thought_signature"),
        _lookup_path(tc, "function", "thought_signature"),
        _lookup_path(tc, "function", "model_extra", "thought_signature"),
    ]
...
def _exception_debug_text(exc: Exception) -> str:
    ...
    if response is not None:
        ...
        if value:
            parts.append(f"response.status_code={value}")
        try:
            body = response.text
        except Exception:
            body = None
        if body:
            parts.append(f"response.text={body[:4000]}")
    ...
    if body:
        try:
            ...
            body_text = str(body)
        parts.append(f"body={body_text[:4000]}")
    return " | ".join(parts)
    ...
    def chat(
        ...
        for msg in messages:
            converted = _message_to_openai(
                msg,
                include_reasoning=self._include_reasoning_history,
            )
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if not retryable or attempt == max_retries:
                    print(
                        f"[openai_compat] non-retryable API error: {_exception_debug_text(exc)}",
                        file=sys.stderr,
                    )
    ...
    def _call_with_stream(self, kwargs: dict[str, Any]) -> Any:
            ...
            if delta.tool_calls:
                for tc_delta in delta.tool_calls:
                    ...
                    if tc_delta.function:
                        ...
                        sig = _extract_thought_signature_from_tool_call(tc_delta)
                        if sig:
                            tool_calls_by_index[idx]["thought_signature"] = sig
        ...
        if tool_calls_by_index:
            ...
            for idx in sorted(tool_calls_by_index):
                ...
                fn.thought_signature = tc.get("thought_signature")
        ...
        if choice.message.tool_calls:
            for tc in choice.message.tool_calls:
                try:
                    ...
                    args = {}
                thought_sig = _extract_thought_signature_from_tool_call(tc)
